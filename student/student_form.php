...
$sql = "Select * from worktbl order by date asc;";
$ResultWork = mysqli_query($link, $sql);
...
function numpeople($num)
{
	if($num == 0)
	{
		...
		return "กลุ่มละ $num คน";
	}
}


function datethai($date_thai)
{
	$day = substr("$date_thai",8,2);
	$month = substr("$date_thai",5,2);
	$month = (int)$month -1;
	$year = substr("$date_thai",0,4);
	$year = $year + 543;
...
function DateDiff($strDate1,$strDate2)
{
	...
	if($ResultDate == 0){
		...
		return "เหลืออีก $ResultDate วัน";
	}
}
				...
				while ($DbarrWork = mysqli_fetch_array($ResultWork))
				{
					if($DbarrWork["class"] == $_SESSION['class'] && $DbarrWork["date"] >= $currentdatetime2 ){
						...
						<div class="ปุ่ม">
							...
							echo DateDiff($currentdatetime2,"$DbarrWork[date]")." หัวข้อ: $DbarrWork[title]  ".numpeople("$DbarrWork[num_people]"); ?></button>
							<div class="panel">
								...
								<?php if($DbarrWork['file'] != 0){ ?>
									...
									<img width="100%" src=/project/teacher/image/<?php echo $DbarrWork['file']; ?>><br>
								 </div>
									<?php
								}
								?>
